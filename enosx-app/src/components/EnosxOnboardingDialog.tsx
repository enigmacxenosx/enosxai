...
export default function EnosxOnboardingDialog({ isOpen, onClose }: EnosxOnboardingDialogProps) {
  ...
  const finish = async () => {
    ...
    try { localStorage.setItem("enosx-onboarding-preferences", JSON.stringify(preferences)); } catch {}
          ...
          >
            ...
            <div className="relative z-10 mt-7 min-h-[280px]">
              <AnimatePresence mode="wait">
                {step === 0 && <motion.div key="language" initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -24 }} className="space-y-5"><div className="flex items-center gap-3"><Globe2 size={22} style={{ color: config.accent }} /><div><h3 className="font-semibold text-white">What language feels natural?</h3><p className="text-xs text-white/40">ENOSX will use this for replies and interface hints.</p></div></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-4">{LANGUAGES.map((item) => <button key={item} onClick={() => setLanguage(item)} className={`rounded-2xl border p-4 text-left text-sm transition hover:-translate-y-1 ${language === item ? "border-white/30 bg-white/15 text-white" : "border-white/10 bg-white/[0.04] text-white/55"}`} style={language === item ? { boxShadow: `0 0 22px rgba(${config.accentRgb},0.2)` } : undefined}>{item}{language === item && <Check size={14} className="mt-2" style={{ color: config.accent }} />}</button>)}</div></motion.div>}
                {step === 1 && <motion.div key="wallpaper" initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -24 }} className="space-y-5"><div className="flex items-center gap-3"><Image size={22} style={{ color: config.accent }} /><div><h3 className="font-semibold text-white">Choose your atmosphere.</h3><p className="text-xs text-white/40">Your wallpaper can be changed anytime.</p></div></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-4">{wallpapers.map((item) => <button key={item.id} onClick={() => setPreset(item.id)} className={`group overflow-hidden rounded-2xl border text-left transition hover:-translate-y-1 ${settings.activePresetId === item.id ? "border-white/50" : "border-white/10"}`}><div className="h-24 bg-cover bg-center" style={{ backgroundImage: `url(${item.thumbnail})` }} /><div className="flex items-center justify-between bg-black/40 px-3 py-2 text-xs text-white/75"><span>{item.label}</span>{settings.activePresetId === item.id && <Check size={13} style={{ color: config.accent }} />}</div></button>)}</div></motion.div>}
                {step === 2 && <motion.div key="identity" initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -24 }} className="space-y-5"><div className="flex items-center gap-3"><UserRound size={22} style={{ color: config.accent }} /><div><h3 className="font-semibold text-white">Tune your experience.</h3><p className="text-xs text-white/40">These choices personalize behavior, not your identity.</p></div></div><div><p className="mb-2 text-[10px] font-bold uppercase tracking-widest text-white/35">Gender (optional)</p><div className="grid grid-cols-2 gap-2 sm:grid-cols-4">{GENDERS.map((item) => <button key={item} onClick={() => setGender(item)} className={`rounded-xl border px-3 py-2 text-xs transition ${gender === item ? "border-white/30 bg-white/12 text-white" : "border-white/10 text-white/50"}`}>{item}</button>)}</div></div><div><p className="mb-2 text-[10px] font-bold uppercase tracking-widest text-white/35">AI personality</p><div className="grid grid-cols-2 gap-2">{PERSONALITIES.map((item) => <button key={item.id} onClick={() => setPersonality(item.id)} className={`rounded-xl border p-3 text-left transition ${personality === item.id ? "border-white/30 bg-white/12" : "border-white/10 bg-white/[0.03]"}`}><span className="block text-sm font-semibold text-white/85">{item.label}</span><span className="text-[11px] text-white/40">{item.desc}</span></button>)}</div></div><div className="grid grid-cols-2 gap-2"><button onClick={() => setNotifications(!notifications)} className={`rounded-xl border p-3 text-left text-xs ${notifications ? "border-white/25 bg-white/10 text-white" : "border-white/10 text-white/45"}`}>Notifications<br /><span className="text-[10px] opacity-60">Helpful updates</span></button><button onClick={() => setCompactMode(!compactMode)} className={`rounded-xl border p-3 text-left text-xs ${compactMode ? "border-white/25 bg-white/10 text-white" : "border-white/10 text-white/45"}`}>Compact mode<br /><span className="text-[10px] opacity-60">More space for chat</span></button></div></motion.div>}
