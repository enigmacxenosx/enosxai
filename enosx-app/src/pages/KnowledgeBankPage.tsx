...
import { useKnowledgeBank, readKnowledgeFile, KnowledgeKind } from "@/hooks/useKnowledgeBank";

const kindOptions: KnowledgeKind[] = ["fact", "instruction", "document", "skill", "project"];

export default function KnowledgeBankPage() {
  const [, navigate] = useLocation();
  const { entries, addEntry, removeEntry, clearAll, importEntries, exportEntries, search, stats } = useKnowledgeBank();
  ...
  const runCommand = () => {
    ...
    const [verb, ...args] = command.split(" "); let response = "";
    if (verb === "help") response = "Commands: push <title> :: <content> | import <file via UI> | export | stats | clear | search <term> | lock";
    else if (verb === "push") {
    ...
    else if (verb === "stats") response = `${stats.total} entries // ${stats.words} words // ${stats.kinds} knowledge types`;
